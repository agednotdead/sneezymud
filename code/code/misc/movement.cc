...
void TBeing::goThroughPortalMsg(const TPortal* o) const {
  ...
  if (type > MAX_PORTAL_TYPE) {
    vlogf(LOG_BUG,
      format("%s has illegal portal type of %d.") % o->shortDescr % type);
    type = 0;
  }
  if (riding) {
    sprintf(buf, "You urge your %s to %s $p.", fname(riding->name).c_str(),
      portal_self_enter_mess[type]);
...
void TBeing::exitFromPortalMsg(const TPortal* o) const {
  ...
  if (type > MAX_PORTAL_TYPE) {
    vlogf(LOG_BUG,
      format("%s has illegal portal type of %d.") % o2->shortDescr % type);
    type = 0;
  }
  if (riding) {
    sprintf(buf, "$n and $s %s %s $p.", fname(riding->name).c_str(),
      portal_self_exit_mess[type]);
...
void portal_flag_change(TPortal* o, unsigned int bit_to_change,
  const char* message, setRemT set) {
  TPortal* o2;
...
bool TBeing::validMove(dirTypeT cmd) {
  ...
  if (riding) {
    ...
    if (!sameRoom(*riding) && (riding->horseMaster() == this)) {
      vlogf(LOG_BUG, format("mount/rider in different rooms: (%s, %s) %d %d") %
                       getName() % riding->getName() % inRoom() %
                       riding->inRoom());
      dismount(POSITION_STANDING);
    }
    if (tbt && !tbt->master) {
      vlogf(LOG_BUG,
        format("Bogus riding situation (no master for %s).  setting to %s") %
          tbt->getName() % getName());
      tbt->master = this;
    }
  }
  ...
  if (IS_SET(exitp->condition, EXIT_CLOSED)) {
    ...
    if (isAffected(AFF_SHADOW_WALK) && !riding) {
      ...
      if (perc > diff) {
        act("$n's transparent body passes through the barrier!", TRUE, this, 0,
          NULL, TO_ROOM, NULL, (isPlayerAction(PLR_STEALTH) ? MAX_MORT : 0));
    ...
    if (!exitp->keyword.empty()) {
      ...
      vlogf(LOG_LOW,
        format("Problematic door: rm %d dir %d (closed with no-name?)") %
          inRoom() % cmd);
      notLegalMove();
      return FALSE;
    }
  ...
  if (!isPc() && rp->isRoomFlag(ROOM_NO_MOB)) {
    if (!master && !(specials.act & ACT_HUNTING) && !rider && !riding &&
        (spec != SPEC_BOUNTY_HUNTER) && (spec != SPEC_LAMPBOY) &&
        (spec != SPEC_JANITOR) && (spec != SPEC_CARAVAN) &&
  ...
  if (!tmon || tmon->mobVnum() != Mob::ELEPHANT ||
  ...
  if (rp->isRoomFlag(ROOM_PEACEFUL)) {
    if (isCombatMode(ATTACK_BERSERK)) {
      act("You can't go into that place of tranquility while berserking.", TRUE,
        this, 0, 0, TO_CHAR);
      return FALSE;
    }
  }
...
bool TBeing::rawMoveTied(dirTypeT dir, int new_r) {
  ...
  if (equipment[WEAR_NECK] && dynamic_cast<THarness*>(equipment[WEAR_NECK]) &&
      tied_to) {
    ...
    if (!tied || !tied->canWear(ITEM_WEAR_TAKE) || dynamic_cast<TTrap*>(tied) ||
        compareWeights(tied->getTotalWeight(TRUE), weight_limit) == -1) {
      if (rider)
        rider->sendTo(COLOR_BASIC,
          format("%s strains against the harness but can't pull %s.\n\r") %
            ((sstring)getName()).cap() % (tied ? tied->getName() : "that"));
      sendTo(COLOR_BASIC,
        format("You strain against the harness but can't pull %s.\n\r") %
          (tied ? tied->getName() : "that"));
      return false;
    }
...
int TBeing::rawMove(dirTypeT dir) {
  ...
  if (isAffected(AFF_CHARM) && master && sameRoom(*master)) {
    ...
    act("You burst into tears at the thought of leaving $N.", FALSE, this, 0,
      master, TO_CHAR);
    return FALSE;
  }
  ...
  if (!(roomp->isUnderwaterSector() || roomp->isWaterSector())) {
    if (isLimbFlags(WEAR_LEG_R, PART_TRANSFORMED)) {
      sendTo(
        "It's hard to walk without legs.\n\rMaybe crawling would help.\n\r");
      return FALSE;
    }
  }
  if (affectedBySpell(AFFECT_PLAYERKILL) && to_here &&
      to_here->isRoomFlag(ROOM_PEACEFUL) && !isImmortal()) {
  ...
  if (!to_here) {
    ...
    sendTo(format("Uh-oh.  The %s melts beneath you as you fall into the "
                  "swirling chaos.\n\r") %
  ...
  if (!riding && !isFlying()) {
    if (bothLegsHurt()) {
      if (!bothArmsHurt() && getPosition() == POSITION_CRAWLING) {
        sendTo(
          format("You drag yourself along the %s slowly and painfully.\n\r") %
          roomp->describeGround());
        need_movement += 20;
        if (eitherArmHurt()) {
          sendTo(
            "And to make matters worse, you only have one arm to drag yourself "
            "with.\n\r");
          need_movement += 20;
        }
    ...
    TThing* stick;
    if ((stick = getStuckIn(WEAR_FOOT_R)) ||
        (stick = getStuckIn(WEAR_FOOT_L))) {
      act("The $o in your foot causes you great pain!", false, this, stick, 0,
        TO_CHAR);
      need_movement += 5;
      if (!::number(0, 4)) {
    ...
    if (getCond(DRUNK) > 9) {
      if (isSwimming()) {
        ...
        if (!::number(0, 4)) {
          sendTo(
            "HEY! One of those purple elephants you keep seeing pushed your "
            "head underwater.\n\r");
          return FALSE;
        }
      } else {
    ...
    if (affectedBySpell(SPELL_HASTE) || affectedBySpell(SPELL_ACCELERATE) ||
        affectedBySpell(SPELL_CHEVAL) || affectedBySpell(SPELL_CELERITE))
  ...
  if (to_here->isWaterSector()) {
    ...
    if (from_here->isAirSector()) {
      ...
      sendTo(
        "Your head breaks the surface, and you take a big refreshing gulp of "
        "air.\n\r");
          ...
          if (!has_boat) {
            if ((rc = canSwim(dir)) > 0) {
              sendTo(format("You swim %s through the water.\n\r") %
                     (isAffected(AFF_SWIM) ? "easily" : "valiantly"));
            } else {
              sendTo("You try to swim, but fail to make any headway.\n\r");
              if (rc == -1)
                sendTo(
                  "The weight of your equipment and inventory makes it too "
                  "difficult.\n\r");
    ...
    if (from_here->isUnderwaterSector()) {
      if (!isAffected(AFF_WATERBREATH)) {
        if ((rc = canSwim(dir)) > 0) {
          ...
          sendTo(
            "You try to swim under the water, but fail to make any "
            "headway.\n\r");
          if (rc == -1)
  ...
  if (!isImmortal() || riding) {
    if (doesKnowSkill(SKILL_HIKING) &&
        (from_here->isMountainSector() || from_here->isSwampSector() ||
  ...
  if (result) {
    ...
    if (followers) {
      for (k = followers; k; k = n) {
        ...
        // sanity check - this should not occur, however the hero faerie proc
        // seems to be causing issues
        if (!tft) {
          vlogf(LOG_BUG,
            format("ERROR: NULL follower for %s in TBeing::moveGroup") %
              getName());
          continue;
        } else {
          ...
          if (!found) {
            vlogf(LOG_BUG,
              format("ERROR: corrupt follower for %s in TBeing::moveGroup.  "
                     "Was this object deleted from followers improperly?") %
                getName());
            return FALSE;
          }
        }
        ...
        if (tft->master != this) {
          ...
          vlogf(LOG_BUG,
            format("ERROR: Bad critter looping through moveGroup()! "
                   "(this=[%s], badguy=[%s], master=[%s])") %
              getName() %
        ...
        if (tft->rider) {
          ...
          for (t = tft->rider; t; t = t2) {
            ...
            if (tb->followers) {
              ...
              // it is possible I was already dismounted, in which case we can
              // skip
              if (!rc && tb->riding) {
                positionTypeT post = tb->getPosition();
                tb->dismount(
                  post <= POSITION_STANDING ? post : POSITION_STANDING);
              }
            } else {
              rc = tb->rawMove(dir);
              if (IS_SET_DELETE(rc, DELETE_THIS)) {
...
#if 1
                // multiple cores generated here, riding (above) yet !riding in
                // dismount() this is temporary. bat 7/9/99
                if (tb->riding)
                  tb->dismount(
                    post <= POSITION_STANDING ? post : POSITION_STANDING);
                else
        ...
        if ((was_in == tft->inRoom()) && !tft->fight() &&
          ...
          if (tft->riding && (tft != tft->riding->horseMaster())) {
...
int TBeing::doMove(dirTypeT cmd) {
  ...
  if (riding && (this != riding->horseMaster())) {
    act("You are not the master of $N, and can't control where $E goes.", FALSE,
      this, 0, riding, TO_CHAR);
    return FALSE;
  }

  if (isAffected(AFF_SHOCKED)) {
    sendTo(
      "You are still recovering from something and cannot bring yourself to "
      "move.\n\r");
    return FALSE;
  }
...
int TBeing::displayMove(dirTypeT dir, int was_in, int total) {
  ...
  if (!rp1 || !rp2) {
    ...
    vlogf(LOG_BUG,
      format("NULL rp in displayMove!  (%s)(%d)") % getName() % was_in);
    return FALSE;
  }
  ...
  *rp1 += *this;
  for (StuffIter it = rp1->stuff.begin(); it != rp1->stuff.end() && (t = *it);
    ...
    if (ch->isImmortal() || ch->canSee(this, INFRA_YES) || makesNoise()) {
      if (isImmortal())
        act(msgVariables(MSG_MOVE_IN, (TThing*)NULL, dirs[dir]), FALSE, this,
          riding, ch, TO_VICT);
      else {
#if 1
        ...
        for (StuffIter it = rp2->stuff.begin(); it != rp2->stuff.end(); ++it) {
          ...
          if (obj->tied_to && obj->tied_to == this) {
            act("$N is pulled out of the room.", TRUE, this, NULL, obj,
              TO_ROOM);
          }
        }
      }
  ...
  *rp2 += *this;
  ...
  if ((dir < 4) || (dir > 5)) {
    if (total == 1) {
      if (riding) {
        sprintf(tmp, "$n has arrived from the %s, riding on $p.",
          dirs[rev_dir(dir)]);
  ...
  for (StuffIter it = rp2->stuff.begin(); it != rp2->stuff.end() && (t = *it);
    ...
    if (tbt->isImmortal() || (tbt->canSee(this, INFRA_YES)) || (makesNoise())) {
      if (isImmortal()) {
        char dirText[256];
        ...
        act(msgVariables(MSG_MOVE_OUT, (TThing*)NULL, dirText), FALSE, this,
          riding, tbt, TO_VICT);
      } else {
#if 1
        ...
        for (StuffIter it = rp2->stuff.begin(); it != rp2->stuff.end(); ++it) {
          ...
          if (obj->tied_to && obj->tied_to == this) {
            act("$N is pulled into the room.", TRUE, this, NULL, obj, TO_ROOM);
          }
        }
      }
    }
...
int TBeing::genericMovedIntoRoom(TRoom* rp, int was_in,
  ...
  // ok, now we have the number of people in my group who are (supposedly)
  // travelling with me we'll use it later to modify how often we call the
  // roomenter mobaggro code -dash
  ...
  if (rp->isRoomFlag(ROOM_DEATH) && !isImmortal()) {
    vlogf(LOG_MISC, format("%s killed by DEATHTRAP at %s (%d)") % getName() %
  ...
  for (StuffIter it = rp->stuff.begin(); it != rp->stuff.end();) {
    ...
    if (was_in != -1) {
      rc = tmons->checkSpec(this, CMD_MOB_MOVED_INTO_ROOM, "",
        reinterpret_cast<TObj*>(static_cast<intptr_t>(
          was_in)));  // casts: int was_in is passed in a pointer field
      if (rc) {
    ...
    if ((groupcount == 1 && (::number(1, 100) < 85)) ||
        (groupcount != 1 && !::number(0, groupcount - 1) &&
          (::number(1, 100) < 66))) {
      // people walking alone, 85%
      // people in groups get 66% / num of people in group (66% chance of
      // aggroing the group)
      // ok here's what were doing - we basically saying he'll only try and
      // aggro once per group. roughly, sort of. its a hack, so sue me.
      ...
      if (IS_SET_DELETE(rc, DELETE_THIS)) {
        ...
        vlogf(LOG_BUG,
          "PAPPY: Suspiciously returning DELETE_THIS from a mob aggro check "
          "which returned DELETE_VICT - are we guaranteed VICT = us?");
        return DELETE_THIS;
      }
  ...
  if (inRoom() == AVENGER_ROOM && hasQuestBit(TOG_AVENGER_HUNTING)) {
    ...
    *rp += *mob;
    ...
    act("$n says, \"So, $N, you think you can defeat me?\"", FALSE, mob, 0,
      this, TO_ROOM);
    act("$n says, \"We shall soon see about that.\"", FALSE, mob, 0, this,
      TO_ROOM);
  }
  ...
  if (in_room == ROOM_VINDICATOR_2 && hasQuestBit(TOG_VINDICATOR_HUNTING_2)) {
    ...
    *rp += *mob;
    act(
      "In front of you, the limbs of a tree start to bend and smoke rises from "
      "the $g.",
      FALSE, mob, 0, 0, TO_ROOM);
    act("With a flash of a blinding light, $n forms from the smoke.", FALSE,
      mob, 0, 0, TO_ROOM);
    act("$n says, \"So, $N, you think you can defeat me?\"", FALSE, mob, 0,
      this, TO_ROOM);
    act("$n says, \"We shall soon see about that.\"", FALSE, mob, 0, this,
...
int AddToCharHeap(TBeing* heap[50], int* top, int total[50], TBeing* k) {
  ...
  if (*top > 50)
    ...
    for (i = 0; ((i < *top) && !found); i++) {
      if (*top > 0) {
        if (dynamic_cast<TMonster*>(k) && (k->number == heap[i]->number) &&
            !heap[i]->shortDescr.empty() &&
  ...
  if (door >= MIN_DIR) {
    ...
    float tRidingManip =
      ((getPosition() == POSITION_MOUNTED)
          ? (1.5 - (((float)getSkillValue(SKILL_RIDE) / 2) / 100))
          : 1.0);
    ...
    if (IS_SET(exitp->condition, EXIT_DESTROYED)) {
      ...
      sendTo(
        "A cave-in blocks the way.  It would take hours to dig your way "
        "through.\n\r");
      return FALSE;
...
int TBeing::doRaise(const char* argument, cmdTypeT cmd) {
  ...
  if (!*type) {
    ...
    sendTo(format("%s: Your attempt to raise it seems to have no effect.\n\r") %
           type);
    return FALSE;
  } else if ((door = findDoor(type, dir, DOOR_INTENT_RAISE, SILENT_NO)) >=
             MIN_DIR) {
    // perhaps it is a door
    exitp = exitDir(door);
    if (exitp->door_type == DOOR_NONE) {
    ...
    if (IS_SET(exitp->condition, EXIT_DESTROYED)) {
      ...
      if (!IS_SET(exitp->condition, EXIT_CLOSED))
        ...
        if (doesKnowSkill(SKILL_DETECT_TRAP)) {
          if (detectTrapDoor(this, door)) {
            sendTo(format("You start to raise the %s, but then notice an "
                          "insidious %s trap...\n\r") %
                   exitp->getName() %
                   sstring(trap_types[exitp->trap_info]).uncap());
            return FALSE;
...
void TBeing::doClose(const char* argument) {
  ...
  if (!*type)
    ...
    sendTo(format("%s: Your attempt to close it seems to have no effect.\n\r") %
           type);
    return;
  } else if ((door = findDoor(type, dir, DOOR_INTENT_CLOSE, SILENT_NO)) >=
             MIN_DIR) {
    if (getPosition() < POSITION_CRAWLING) {
      sendTo("You need to stand up to reach it.\n\r");
      doStand();
    ...
    if (IS_SET(exitp->condition, EXIT_DESTROYED)) {
      sendTo(
        format("The %s has been destroyed, it can't be closed anymore.\n\r") %
        exitp->getName());
      return;
    } else if (IS_SET(exitp->condition, EXIT_CAVED_IN)) {
      sendTo(
        "It's been caved in.  You can't get much more closed than that...\n\r");
      return;
    } else if (IS_SET(exitp->condition, EXIT_CLOSED))
...
int TBeing::doLower(const char* argument) {
  ...
  if (!*type) {
    ...
    sendTo(format("%s: Your attempt to lower it seems to have no effect.\n\r") %
           type);
    return FALSE;
  } else if ((door = findDoor(type, dir, DOOR_INTENT_LOWER, SILENT_NO)) >=
             MIN_DIR) {
    exitp = exitDir(door);
    if (exitp->door_type == DOOR_NONE) {
      sendTo("That's impossible, I'm afraid.\n\r");
    ...
    if (IS_SET(exitp->condition, EXIT_DESTROYED)) {
      ...
      sendTo(
        "It's been caved in.  You can't get much more lowered than "
        "that...\n\r");
      return FALSE;
    } else if (exitp->door_type == DOOR_DRAWBRIDGE) {
      ...
      if (!IS_SET(exitp->condition, EXIT_CLOSED))
        ...
        if (doesKnowSkill(SKILL_DETECT_TRAP)) {
          if (detectTrapDoor(this, door)) {
            sendTo(format("You start to lower the %s, but then notice an "
                          "insidious %s trap...\n\r") %
                   exitp->getName() %
                   sstring(trap_types[exitp->trap_info]).uncap());
            return FALSE;
...
void TBeing::doLock(const char* argument) {
  ...
  if (!*type)
    sendTo("Lock what?\n\r");
           ...
           !dynamic_cast<TBaseCorpse*>(obj) && !dynamic_cast<TKey*>(obj)) {
    // this is an object */
    obj->lockMe(this);
    ...
    sendTo(format("%s: Your attempt to lock it seems to have no effect.\n\r") %
           type);
    return;
  } else if ((door = findDoor(type, dir, DOOR_INTENT_LOCK, SILENT_NO)) >=
             MIN_DIR) {
    exitp = exitDir(door);
    ...
    if (IS_SET(exitp->condition, EXIT_DESTROYED)) {
      sendTo(
        format(
          "The %s has been destroyed; locking it now is a bit silly.\n\r") %

        exitp->getName().c_str());
      return;
    } else if (IS_SET(exitp->condition, EXIT_CAVED_IN)) {
      sendTo(
        "How do you expect to find the lock when it's buried under that cave "
        "in?\n\r");
      return;
    } else if (!IS_SET(exitp->condition, EXIT_CLOSED))
...
void TBeing::doUnlock(const char* argument) {
  ...
  if (!*type)
    ...
    // this is an object */
    obj->unlockMe(this);
  } else if ((findDoor(type, dir, DOOR_INTENT_UNLOCK, SILENT_YES) ==
               DIR_NONE) &&
             roomp && roomp->ex_description &&
             (tmpdesc = roomp->ex_description->findExtraDesc(argument))) {
    sendTo(
      format("%s: Your attempt to unlock it seems to have no effect.\n\r") %
      type);
    return;
  } else if ((door = findDoor(type, dir, DOOR_INTENT_UNLOCK, SILENT_NO)) >=
             MIN_DIR) {
    exitp = exitDir(door);
    ...
    if (IS_SET(exitp->condition, EXIT_DESTROYED)) {
      ...
      if (fight()) {
        act("You are forced to sacrifice a few rounds to unlock the $T.", TRUE,
          this, 0, reinterpret_cast<const TThing*>(exitp->getName().c_str()),
          TO_CHAR);
        cantHit += loseRound(5);
      }
      char buf[256];
      ...
      if (realObj < 0)
        return;
      snprintf(buf, cElements(buf), "$n unlocks the $T with %s.",
        obj_index[realObj].short_desc);
      act(buf, TRUE, this, 0,
        reinterpret_cast<const TThing*>(exitp->getName().c_str()), TO_ROOM);
      snprintf(buf, cElements(buf), "You unlock the $T with %s.",
        obj_index[realObj].short_desc);
      act(buf, TRUE, this, 0,
...
int TBeing::doEnter(const char* argument, TPortal* por) {
  ...
  if (*buf) {
    ...
    for (door = MIN_DIR; door < MAX_DIR; door++) {
      if (canGo(door) &&
          real_roomp(exitDir(door)->to_room)->isRoomFlag(ROOM_INDOORS)) {
        rc = doMove(door);
        if (IS_SET_DELETE(rc, DELETE_THIS))
          return DELETE_THIS;
        return FALSE;
...
void TBeing::doStand() {
  ...
  switch (getPosition()) {
    ...
    case POSITION_RESTING:
      ...
      act("$n stops resting and clambers to $s feet.", TRUE, this, 0, 0,
        TO_ROOM);
      if (riding)
        dismount(POSITION_STANDING);
      setPosition(POSITION_STANDING);
    ...
    case POSITION_MOUNTED:
      ...
      break;
    default:
      act("You stop floating around and put your feet on the $g.", FALSE, this,
        0, 0, TO_CHAR);
...
void TThing::sitMe(TBeing* ch) { ch->sendTo("You can't sit in that.\n\r"); }
  ...
  if (arg.empty() && (!riding || dynamic_cast<TBeing*>(riding))) {
    ...
    switch (getPosition()) {
      ...
      default:
        ...
        act("You stop floating around, and sit down.", FALSE, this, 0, 0,
          TO_CHAR);
...
void TThing::restMe(TBeing* ch) { ch->sendTo("You can't rest in that.\n\r"); }
...
void TBeing::doRest(const sstring& argument) {
  ...
  if (arg.empty() && (!riding || dynamic_cast<TBeing*>(riding))) {
    ...
    switch (getPosition()) {
      case POSITION_STANDING:
      case POSITION_CRAWLING:
        act("You sit down, lean back and rest your tired bones.", FALSE, this,
          0, 0, TO_CHAR);
      ...
      case POSITION_SITTING:
        ...
        act("You lean back and rest your tired bones.", FALSE, this, 0, 0,
          TO_CHAR);
  ...
  if (arg.empty() && (!riding || dynamic_cast<TBeing*>(riding))) {
    ...
    switch (getPosition()) {
      ...
      case POSITION_MOUNTED:
        ...
        break;
      default:
        act("You stop floating around, and lie down to sleep.", FALSE, this, 0,
          0, TO_CHAR);
...
void TBeing::doWake(const char* argument) {
  ...
  if (*arg) {
    if (getPosition() == POSITION_SLEEPING)
      act("You can't wake people up if you are asleep yourself!", FALSE, this,
        0, 0, TO_CHAR);
    else {
      tmp_char = get_char_room_vis(this, arg);
      if (tmp_char) {
        if (tmp_char == this)
          act("If you want to wake yourself up, just type 'wake'", FALSE, this,
            0, 0, TO_CHAR);
        else {
          if (tmp_char->getPosition() == POSITION_SLEEPING) {
            if (tmp_char->isAffected(AFF_SLEEP))
              ...
              if (tmp_char->checkBlackjack()) {
                ...
                act("You are awakened and drug to your feet by $n.", TRUE, this,
                  0, tmp_char, TO_VICT);
              } else {
                act("You wake $M up.", FALSE, this, 0, tmp_char, TO_CHAR);
                tmp_char->setPosition(POSITION_SITTING);
                act("You are awakened by $n.", TRUE, this, 0, tmp_char,
                  TO_VICT);
                tmp_char->stopsound();
              }
            }
    ...
    if (isAffected(AFF_SLEEP))
      ...
      if (getPosition() > POSITION_SLEEPING)
        ...
        if (checkBlackjack()) {
          ...
          act("You wake, climb out of the $o and stand up.", TRUE, this, riding,
            0, TO_CHAR);
          act("$n awakens, climbs out of the $o and stands up.", TRUE, this,
            riding, 0, TO_ROOM);
          dismount(POSITION_STANDING);
          setPosition(POSITION_STANDING);
        } else {
...
void TBeing::doFollow(const char* argument) {
  ...
  if ((this != leader) && !isImmortal()) {
    ...
    if (leader->isPlayerAction(PLR_GRPQUEST) && !isPlayerAction(PLR_GRPQUEST)) {
      sendTo(format("COLOR_MOBS, %s has a group quest flag. You can't follow "
                    "without one yourself.\n\r") %
             leader->getName());
      return;
    }
    if (isPlayerAction(PLR_GRPQUEST) && !leader->isPlayerAction(PLR_GRPQUEST)) {
      sendTo(
        "You can't follow someone without a group quest flag, while you have "
        "that flag.\n\r");
      return;
    }
  }
  if (isAffected(AFF_CHARM) && master)
    act("But you only feel like following $N!", FALSE, this, 0, master,
      TO_CHAR);
  else {
    if (leader == this) {
      if (!master) {
      ...
      if (circleFollow(leader)) {
        act("Sorry, but following in 'loops' is not allowed.", FALSE, this, 0,
          0, TO_CHAR);
        return;
      }
      if (master)
...
void TBeing::setPosition(positionTypeT pos) {
  ...
  if (rider && pos < POSITION_RESTING) {
    act("Your mount's change in position forces you off.", FALSE, rider, NULL,
      this, TO_CHAR);
    act("$n is forced off $N.", FALSE, rider, NULL, this, TO_ROOM);

    rider->dismount((!::number(0, 9) ? POSITION_STANDING : POSITION_SITTING));
  }

  if (dynamic_cast<TBeing*>(riding) && (pos != POSITION_FIGHTING) &&
      (pos != POSITION_MOUNTED)) {
    // for debug
    // vlogf(LOG_BUG, format("Mounted person (%s) set to new position (%s:%d).")
    // %  getName() % position_types[pos] % pos); this is called whenever a mob
    // falls off a mount - dismount them first
    dismount(POSITION_STANDING);
  }
  if (!hasLegs()) {
    if ((pos == POSITION_SLEEPING) || (pos == POSITION_RESTING) ||
        (pos == POSITION_SITTING) || (pos == POSITION_CRAWLING))
      return;
  }
  if (task)
    stopTask();
  ...
  if (!riding) {
    switch (getPosition()) {
      ...
      case POSITION_SITTING:
        ...
        act("$n gets down on $s knees and starts to crawl.", TRUE, this, 0, 0,
          TO_ROOM);
    ...
    if (!IS_SET(exitp->condition, EXIT_CLOSED)) {
      ...
      float tRidingManip =
        ((getPosition() == POSITION_MOUNTED)
            ? (1.5 - (((float)getSkillValue(SKILL_RIDE) / 2) / 100))
            : 1.0);
      if ((exitp->weight * tRidingManip) >
          maxWieldWeight(NULL, HAND_TYPE_PRIM)) {
        sendTo(
          format("The %s is too large and heavy for you to budge it.\n\r") %
          exitp->getName());
        sprintf(buf, "$n throws $mself at a %s, but $e can't budge it.",
          exitp->getName().c_str());
        act(buf, TRUE, this, 0, 0, TO_ROOM);
      ...
      if (real_roomp(exitp->to_room) &&
          real_roomp(exitp->to_room)->roomIsEmpty(true)) {
        rawOpenDoor(dir);
        return 0;
      }
    }
...
void TBeing::doFly() {
  ...
  if (roomp && roomp->isUnderwaterSector()) {
    sendTo(
      "It hurts your brain too much even contemplating how to fly "
      "underwater?!?\n\r");
    return;
  }
  if (race->isWinged() && race->isFeathered() &&
      !isAffected(AFF_FLIGHTWORTHY) && !isAffected(AFF_FLYING) &&
      !isImmortal()) {
    sendTo(
      "Your flight feathers are too dirty to fly properly.  You need to keep "
      "up on your preening.\n\r");
    return;
  }
  act("You take to the air and start flying about.", TRUE, this, 0, 0, TO_CHAR);
  act("$n takes to the air and starts flying about.", TRUE, this, 0, 0,
    TO_ROOM);
...
int TBeing::doMortalGoto(const sstring& argument) {
  ...
  if (!inGrimhaven()) {
    sendTo(
      "You aren't familiar with this area and have no idea how to get "
      "there.\n\r");
    return FALSE;
  }
  ...
  if (roomp && (roomp->getLight() + visionBonus <= 0) &&
  ...
  if (in_room == 634) {
    ...
    sendTo(
      "Once you have entered the Guild, you may use goto to continue.\n\r");
    return FALSE;
  } else if (is_abbrev(arg, "fairfight")) {
    auto path = pathfind(*this, findFairFight(this),
      "There's already a fair fight here.\n\r");
    if (!path)
      return FALSE;
    ...
    is_abbrev(arg, "air_trainer") || is_abbrev(arg, "air-trainer") ||
    is_abbrev(arg, "alchemy_trainer") || is_abbrev(arg, "alchemy-trainer") ||
    is_abbrev(arg, "earth_trainer") || is_abbrev(arg, "earth-trainer") ||
    is_abbrev(arg, "fire_trainer") || is_abbrev(arg, "fire-trainer") ||
    is_abbrev(arg, "sorcery_trainer") || is_abbrev(arg, "sorcery-trainer") ||
    is_abbrev(arg, "spirit_trainer") || is_abbrev(arg, "spirit-trainer") ||
    is_abbrev(arg, "water_trainer") || is_abbrev(arg, "water-trainer") ||
    is_abbrev(arg, "wrath_trainer") || is_abbrev(arg, "afflictions_trainer") ||
    is_abbrev(arg, "cures_trainer") || is_abbrev(arg, "hand_trainer") ||
    is_abbrev(arg, "looting_trainer") || is_abbrev(arg, "murder_trainer") ||
    is_abbrev(arg, "dueling_trainer") || is_abbrev(arg, "wizardry_trainer") ||
    is_abbrev(arg, "faith_trainer") || is_abbrev(arg, "slash_trainer") ||
    is_abbrev(arg, "blunt_trainer") || is_abbrev(arg, "pierce_trainer") ||
    is_abbrev(arg, "ranged_trainer") || is_abbrev(arg, "meditation_trainer") ||
    is_abbrev(arg, "survival_trainer") || is_abbrev(arg, "nature_trainer") ||
    is_abbrev(arg, "animal_trainer") || is_abbrev(arg, "aegis_trainer") ||
    is_abbrev(arg, "plants_trainer") || is_abbrev(arg, "soldiering_trainer") ||
    is_abbrev(arg, "blacksmithing_trainer") ||
    is_abbrev(arg, "fighting_trainer") || is_abbrev(arg, "mounted_trainer") ||
    is_abbrev(arg, "leverage_trainer") || is_abbrev(arg, "mindbody_trainer") ||
    is_abbrev(arg, "focused_trainer") || is_abbrev(arg, "barehand_trainer") ||
    is_abbrev(arg, "poisons_trainer") || is_abbrev(arg, "healing_trainer") ||
    is_abbrev(arg, "undead_trainer") || is_abbrev(arg, "totemism_trainer") ||
    is_abbrev(arg, "stealth_trainer") || is_abbrev(arg, "traps_trainer") ||
    is_abbrev(arg, "wrath-trainer") || is_abbrev(arg, "afflictions-trainer") ||
    is_abbrev(arg, "cures-trainer") || is_abbrev(arg, "hand-trainer") ||
    is_abbrev(arg, "looting-trainer") || is_abbrev(arg, "murder-trainer") ||
    is_abbrev(arg, "dueling-trainer") || is_abbrev(arg, "wizardry-trainer") ||
    is_abbrev(arg, "faith-trainer") || is_abbrev(arg, "slash-trainer") ||
    is_abbrev(arg, "blunt-trainer") || is_abbrev(arg, "pierce-trainer") ||
    is_abbrev(arg, "ranged-trainer") || is_abbrev(arg, "meditation-trainer") ||
    is_abbrev(arg, "survival-trainer") || is_abbrev(arg, "nature-trainer") ||
    is_abbrev(arg, "animal-trainer") || is_abbrev(arg, "aegis-trainer") ||
    is_abbrev(arg, "plants-trainer") || is_abbrev(arg, "soldiering-trainer") ||
    is_abbrev(arg, "blacksmithing-trainer") ||
    is_abbrev(arg, "fighting-trainer") || is_abbrev(arg, "mounted-trainer") ||
    is_abbrev(arg, "leverage-trainer") || is_abbrev(arg, "mindbody-trainer") ||
    is_abbrev(arg, "focused-trainer") || is_abbrev(arg, "barehand-trainer") ||
    is_abbrev(arg, "poisons-trainer") || is_abbrev(arg, "frog-trainer") ||
    is_abbrev(arg, "spider-trainer") || is_abbrev(arg, "totemism-trainer") ||
    is_abbrev(arg, "stealth-trainer") || is_abbrev(arg, "traps-trainer")) {
    sendTo("That discipline is fairly advanced.\n\r");
    sendTo(
      "You will need to explore in order to locate the trainer for that "
      "discipline.\n\r");
    return FALSE;
  } else {
    ...
    for (auto pair : locations) {  // no map lookup :( because of is_abbrev. The
                                   // map is still good for schema.
    ...
    if (!targ_rm) {
      static std::map<const char*, int> npcs = {{"karalina", 343},
        {"mage", 200}, {"cleric", 201}, {"warrior", 202}, {"fighter", 202},
        {"thief", 203}, {"thieving", 203}, {"ranger", 204}, {"deikhan", 205},
        {"shaman", 206}, {"monk", 207}, {"lore_trainer", 333},
        {"lore-trainer", 333}, {"theology_trainer", 335},
        {"theology-trainer", 335}, {"mage_trainer", 502}, {"mage-trainer", 502},
        {"cleric_trainer", 259}, {"cleric-trainer", 259},
        {"thief_trainer", 262}, {"thief-trainer", 262},
        {"warrior_trainer", 355}, {"warrior-trainer", 355},
        {"shaman_trainer", 247}, {"shaman-trainer", 247},
        {"deikhan_trainer", 278}, {"deikhan-trainer", 278},
        {"ranger_trainer", 261}, {"ranger-trainer", 261}, {"monk_trainer", 285},
        {"monk-trainer", 285}, {"adventuring_trainer", 290},
        {"adventuring-trainer", 290}, {"combat_trainer", Mob::COMBAT_TRAINER},
        {"combat-trainer", Mob::COMBAT_TRAINER}};

      for (auto pair : npcs) {  // no map lookup :( because of is_abbrev. The
                                // map is still good for schema.
        if (is_abbrev(arg, pair.first)) {
  ...
  auto path = pathfind(*this, findRoom(targ_rm),
...
void TObj::closeMe(TBeing* ch) { ch->sendTo("That's not a container.\n\r"); }

void TObj::lockMe(TBeing* ch) { ch->sendTo("That's not a container.\n\r"); }

void TObj::unlockMe(TBeing* ch) { ch->sendTo("That's not a container.\n\r"); }
