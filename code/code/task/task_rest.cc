        ...
        if (!ch->roomp->isRoomFlag(ROOM_NO_HEAL)) {
          ...
          if (ch->hasClass(CLASS_SHAMAN) &&
            ...
            if (ch->hasClass(CLASS_THIEF) &&
                ch->isAffected(SKILL_CONCEALMENT)) {
              mod *= 1.5;
              if (percentChance(20)) {
                ch->sendTo(
                  "You find comfort in being hidden from the world around "
                  "you.\n\r");
              }
            }
            if (ch->roomp->hasMobToCuddle()) {
              ...
              if (percentChance(20)) {
                ch->sendTo(
                  "Your furry companion brings you a feeling of security.\n\r");
              }
            }
            if (ch->roomp->hasCampfire()) {
              ...
              if (percentChance(20)) {
                ch->sendTo(
                  "The warmth of the campfire makes you feel better.\n\r");
              }
            }
            ch->addToHit(1 * mod);
          }
